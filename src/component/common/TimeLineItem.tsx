...
export default function TimeLineItem({ timeline }: Props) {
  ...
  return (
    <>
      ...
      >
        <span
          className={`timeline-item-dot absolute top-1/2 z-20 h-[8px] w-[8px] -translate-x-1/2 -translate-y-1/2 rounded-full transition-colors duration-300 md:left-[calc(50%+1px)] ${isInView ? 'bg-black' : 'bg-gray-400'}`}
        />
        ...
        >
          ...
          <p className="mt-2 text-xl">{timeline.desc}</p>
        </motion.div>
      </li>
    </>
  );
